...
// Función para escuchar cambios en tiempo real y actualizar el estado de las mesas sin recargar la página
function escucharCambios() {
  const contenedor = document.getElementById("contenedorMesas");
  const mesasRef = doc(db, "mesas");

  // Escuchar cambios en la base de datos en tiempo real
  db.collection("mesas").onSnapshot((querySnapshot) => {
    // Limpiar el contenedor de mesas para actualizar
    contenedor.innerHTML = "";

    // Volver a cargar las mesas con los datos actualizados
    cargarMesas();
  });
}
